            ...
            <table class="w-full text-left border-separate border-spacing-0" :class="{'opacity-50 pointer-events-none': isFetching}">
                <thead class="text-xs font-semibold text-slate-500 dark:text-zinc-400 sticky top-0 bg-slate-50 dark:bg-[#151518] z-30 shadow-[0_1px_0_0_#e2e8f0] dark:shadow-[0_1px_0_0_#27272a] select-none">
                    <tr>
                        ...
                        <th @click="toggleSort('sensor_id')" class="px-4 py-3 cursor-pointer hover:text-slate-800 dark:hover:text-zinc-200 transition-colors group" role="button" tabindex="0" aria-label="Sort by sensor id">
                        ...
                        <th @click="toggleSort('node_id')" class="px-4 py-3 text-right cursor-pointer hover:text-slate-800 dark:hover:text-zinc-200 transition-colors group" role="button" tabindex="0" aria-label="Sort by node">
                            <div class="flex items-center justify-end gap-1.5">Node
                                <svg class="w-3 h-3 transition-transform duration-200" :class="[sortCol === 'node_id' ? 'opacity-100 text-blue-500 dark:text-zinc-300' : 'opacity-0 group-hover:opacity-50 text-slate-400 dark:text-zinc-500', isDownArrow('node_id') ? 'rotate-180' : '']" viewBox="0 0 384 512" fill="currentColor"><path d="M214.6 41.4c-12.5-12.5-32.8-12.5-45.3 0l-160 160c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 141.2V448c0 17.7 14.3 32 32 32s32-14.3 32-32V141.2L329.4 246.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3l-160-160z"></path></svg>
                            </div>
                        </th>
                ...
                <tbody class="relative z-0">
                    ...
                    <template v-for="event in paginatedEvents" :key="event.id">
                        <tr class="hover:bg-slate-50 dark:hover:bg-[#18181b] cursor-pointer transition-colors relative z-0 group"
                            ...
                            <td class="px-4 py-3 text-sm text-slate-500 dark:text-zinc-400 mono max-w-[140px] md:max-w-[200px] lg:max-w-sm xl:max-w-xl 2xl:max-w-none truncate" :class="expandedRows.has(event.id) ? 'border-b border-transparent' : 'border-b border-slate-200 dark:border-zinc-800/50'" :title="event.sensor_id">{{ event.sensor_id }}</td>
                            <td class="px-4 py-3 text-sm text-right text-blue-600 dark:text-zinc-300 font-medium mono max-w-[140px] md:max-w-[200px] lg:max-w-sm xl:max-w-xl 2xl:max-w-none truncate" :class="expandedRows.has(event.id) ? 'border-b border-transparent' : 'border-b border-slate-200 dark:border-zinc-800/50'" :title="event.node_id || 'Unassigned'">{{ event.node_id || 'Unassigned' }}</td>
                                        ...
                                        <div class="flex flex-wrap gap-x-6 gap-y-6">
                                            <div v-for="(val, key) in event.details" :key="key" class="flex flex-col group w-fit min-w-[120px] max-w-full">
                                                ...
                                                <div v-if="getDataType(val) === 'primitive_array'" class="flex flex-wrap gap-2">
                                                    <span v-for="(item, i) in val" :key="i" class="px-2 py-1 bg-slate-100 dark:bg-zinc-800/40 border border-slate-200 dark:border-zinc-700/50 rounded text-[11px] text-slate-700 dark:text-zinc-300 mono break-all shadow-sm">
                                                        {{ String(item) }}
                                                    </span>
                                                </div>
