...
const formatEnvVarName = (name: string) => {
  if (name.startsWith('HW_')) name = name.substring(3)
        ...
        <transition enter-active-class="transition duration-normal ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-[var(--duration-fast)] ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
            <div v-if="sensor && show" class="fixed inset-0 z-[var(--z-modal)] flex justify-center items-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm" @mousedown.self="$emit('close')">
                <div class="bg-bg-base w-full max-w-3xl h-full max-h-[85vh] rounded-lg shadow-2xl flex flex-col overflow-hidden border border-border-default transform transition-all">
                    ...
                    <div class="flex-1 overflow-hidden bg-bg-base flex flex-col">
                        <div v-show="activeTab === 'readme'" class="flex-1 overflow-y-auto custom-scroll p-6 md:p-8 readme-container text-sm text-text-m"><p class="mb-6 text-sm font-medium text-text-h">{{ sensor.documentation?.summary }}</p><div v-for="section in sensor.documentation?.sections" :key="section.title" class="mb-6"><h3 class="text-sm font-semibold text-text-h mb-3">{{ section.title }}</h3><ul v-if="section.type === 'list'" class="list-disc pl-5 space-y-1"><li v-for="item in section.content" :key="item">{{ item }}</li></ul></div></div>
                        <form v-show="activeTab === 'config'" @submit.prevent="$emit('apply', Object.fromEntries(Object.entries(envVarValues).map(([k, v]) => [k, v !== undefined && v !== null ? String(v) : ''])))" class="flex-1 flex flex-col overflow-hidden">
                            <div class="flex-1 overflow-y-auto custom-scroll p-6 md:p-8 flex flex-col">
                                <div class="mb-6"><h4 class="text-sm font-semibold text-text-h mb-3">Sensor Settings</h4>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <template v-for="env in sortedEnvVars" :key="env.name"><BaseInput v-if="env.name !== 'HW_SEVERITY'" v-model="envVarValues[env.name]" :type="getEnvType(env)" :label="formatEnvVarName(env.name)" :description="env.description" :placeholder="getUIDefault(env.default)" :defaultFallback="getUIDefault(env.default)" @focus="activeEnvVar = env.name" @blur="activeEnvVar = null" /></template>
                                    </div>
                                </div>
                            ...
                            <div class="mt-auto border-t border-border-default p-4 flex justify-end shrink-0 bg-bg-surface"><BaseButton variant="primary" type="submit" class="px-6">{{ isEditing ? 'Apply Settings' : 'Add to Node' }}</BaseButton></div>
                        </form>
                    </div>
                </div>
            </div>
