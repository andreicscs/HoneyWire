    ...
    <aside class="flex flex-col bg-bg-base border-r border-border-default z-10 transition-all duration-150 ease-in-out shrink-0"
        ...
        <div class="h-14 flex items-center px-[18px] shrink-0 mb-2">
            <button @click="appStore.sidebarOpen = !appStore.sidebarOpen" 
                    type="button"
                    class="p-1.5 rounded-md text-secondary-text hover:text-text-h hover:bg-secondary-hover transition-colors outline-none">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
            </button>
        </div>

        <nav class="flex-1 space-y-2 overflow-y-auto custom-scroll overflow-x-hidden px-3">
            <div class="px-3 text-xs  text-text-m transition-all duration-150 overflow-hidden whitespace-nowrap"
                 :class="sidebarOpen ? 'max-h-6 opacity-100 mb-1' : 'max-h-0 opacity-0 mb-0'">Menu</div>
            
            <button @click="appStore.currentView = 'dashboard'" 
                    type="button"
                    class="w-full flex items-center px-3 py-2.5 rounded-md text-base transition-all border outline-none"
                    :class="currentView === 'dashboard' ? 'bg-secondary-selected text-text-h  shadow-sm border-secondary-border' : 'border-transparent text-secondary-text  hover:bg-secondary-hover hover:text-text-h'"
            ...
            <button @click="appStore.currentView = 'store'" 
                    type="button"
                    class="w-full flex items-center px-3 py-2.5 rounded-md text-base transition-all border outline-none"
                    :class="currentView === 'store' ? 'bg-secondary-selected text-text-h  shadow-sm border-secondary-border' : 'border-transparent text-secondary-text  hover:bg-secondary-hover hover:text-text-h'"
            ...
            <button @click="appStore.currentView = 'settings'" 
                    type="button"
                    class="w-full flex items-center px-3 py-2.5 rounded-md text-base transition-all border outline-none"
                    :class="currentView === 'settings' ? 'bg-secondary-selected text-text-h  shadow-sm border-secondary-border' : 'border-transparent text-secondary-text  hover:bg-secondary-hover hover:text-text-h'"
