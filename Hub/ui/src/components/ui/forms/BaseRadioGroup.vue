...
defineProps({
    ...
    options: { type: Array, required: true }, // e.g. ['ntfy', 'slack']
...
<template>
    <div>
        <label v-if="label" class="block text-xs  text-text-m  tracking-wider mb-2">
            {{ label }}
        </label>
        ...
        <p v-if="description" class="text-sm text-text-m mt-3">{{ description }}</p>
    </div>
</template>