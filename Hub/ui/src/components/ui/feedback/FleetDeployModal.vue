        ...
        <div v-else class="space-y-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    <code class="flex-1 block px-3 py-2.5 bg-bg-inset border border-border-default rounded-md text-sm font-mono text-text-h truncate select-all">{{ nodeKey }}</code>
                </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    ...
                    <BaseButton variant="ghost" class="!py-1 !px-2 !text-xs transition-colors" :class="copiedStates['modal-cmd'] ? '!text-success-main hover:!text-success-main' : ''" @click="handleCopy('modal-cmd', `curl -sL https://hub.honeywire.local/wizard.sh | bash -s -- --key ${nodeKey}`)">
                        <span class="flex items-center gap-1.5">{{ copiedStates['modal-cmd'] ? 'Copied!' : 'Copy' }}</span>
                    </BaseButton>
                </div>
                <code class="block w-full p-4 bg-bg-inset border border-border-default rounded-md text-sm font-mono text-text-m whitespace-pre-wrap break-all leading-normal select-all">curl -sL https://hub.honeywire.local/wizard.sh | bash -s -- --key {{ nodeKey }}</code>
            </div>
