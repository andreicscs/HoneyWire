                ...
                <template #header>
                    <div class="flex items-start justify-between w-full">
                        ...
                        <BaseMeatballMenu v-if="node.id !== 'unassigned'" :id="`node-menu-${node.id}`">
                            <button @click="handleSilenceNode(node.id)" 
                                    ...
                                    :class="node.isSilenced ? 'text-archive-text hover:bg-archive-bg' : ' hover:text-text-h'">
                            ...
                            <button @click="handleForgetNode(node.id)" class="w-full text-left px-3 py-2 text-sm font-medium text-danger-text flex items-center gap-2 hover:bg-danger-bg transition-colors group border-t border-border-default mt-1 pt-2">
                                ...
                                <span v-if="fleetStore.isNodeActionPending(node.id, 'deleting')">Deleting...</span><span v-else>Delete</span>
                            </button>
                        </BaseMeatballMenu>
                    </div>
                </template>
