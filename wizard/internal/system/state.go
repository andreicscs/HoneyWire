...
func ValidateComposeConfig(composeData []byte) error {
	var compose struct {
	...
	for serviceName, service := range compose.Services {
		for _, rawPort := range service.Ports {
			hostPort, ok := parseHostPort(rawPort)
			if !ok {
				continue
			}
			if owner, exists := portOwners[hostPort]; exists {
				return fmt.Errorf("port conflict detected: host port %d defined by services %q and %q", hostPort, owner, serviceName)
			}
			portOwners[hostPort] = serviceName
		}

		if envList, ok := service.Environment.([]interface{}); ok {
			seen := make(map[string]bool)
			for _, envItem := range envList {
				envStr, ok := envItem.(string)
				if !ok {
					continue
				}
				key := envStr
				if idx := strings.Index(envStr, "="); idx != -1 {
					key = envStr[:idx]
				}
				if key == "" {
					continue
				...
				if seen[key] {
					return fmt.Errorf("environment conflict detected: duplicate variable %q in service %q", key, serviceName)
				}
				seen[key] = true
			}
		}
...
func parseHostPort(raw interface{}) (int, bool) {
	switch v := raw.(type) {
	case int:
		return v, true
	case int64:
